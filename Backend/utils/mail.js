...
function createEmailBody(orderData) {
  ...
  const htmlLink = `http://localhost:5000/api/v1/order/order/responseForm/${orderId}`;
  const emailBody = `
  <!DOCTYPE html>
  <html>
  <head>
  ...
  <body>
    <div class="container">
      ...
      <p><strong>Please Copy the Order ID and reachout to the below link within next 6 hours.</strong></p>
      <div>
        <a class="btn" href="${htmlLink}"><p>Click here to approve or reject the order</p></a>
      </div>
    </div>
  </body>
  </html>
...
async function sendEmail(orderData, senderEmail){
  console.log("in mail");
...
<p><strong>Please Copy the Order ID and reachout to the below link within next 6 hours.</strong></p>
<div>
  <a href="${htmlLink}">Click here to approve or reject the order</a>
</div> */