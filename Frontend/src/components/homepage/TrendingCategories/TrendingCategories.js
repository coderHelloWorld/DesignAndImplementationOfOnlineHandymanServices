...
import classes from './trendingcategories.module.css'
import { useNavigate } from 'react-router-dom';

function TrendingCategories(props){
    console.log("props data in trendingCategory",props.datas);
    // let data_members = [
    //     {
    //         id:1,
    //         name: "Electrician"
    ...
    //     }
    // ]
    let data_members = props.datas.slice(0,3);
    ...
    const listItems = data_members.map((item,index) => (
        <div key={item.id} className={classes[`member_${(index+1)}`]} onClick={() => handleItemClickt(item.id)}>
    ...
    return(
        <div ref={ref} className={classes.topdiv}>
            {isIntersecting && (<h1 className={classes.h1}> Trending Categories </h1>)}
